...
const AddSekolahKristenModal = ({ setIsOpenModalAdd, getSekolahKristen }) => {
  ...
  const [SKIjin, setSKIjin] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://192.168.85.20:3005/sekolah-kristen", {
        ...
        sk_ijin: SKIjin,
        no_reqPendirian: NoHerRegistrasiPendirian,
      ...
      getSekolahKristen(statusSekolah);
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <div className="w-full bg-white rounded-lg shadow-lg h-full inline-block">
          ...
          <div className="p-4 space-y-4 inline-block h-[75%] overflow-y-scroll">
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-5 mb-2">
                <label htmlFor="skIjin" className="label-input">
                  SK Ijin Pendirian
                </label>
                <input
                  value={SKIjin}
                  onChange={(e) => setSKIjin(e.target.value)}
                  type="text"
                  id="skIjin"
                  className="w-full input"
                />
...
export default AddSekolahKristenModal;
